package com.retiredroca.craftingnetwork.recipe;
...
public class CraftingUpgradeRecipe implements CraftingRecipe {
    public static final String TAG_TIER = "tier";

    private final int targetTier;

    public CraftingUpgradeRecipe(int targetTier) {
    ...
    public boolean matches(CraftingInput input, Level level) {
        ...
        if (!center.is(CraftingNetworkCommon.platform().craftingStationItem())) {
            return false;
        }
    ...
    private ItemStack buildResult() {
        ItemStack result = new ItemStack(CraftingNetworkCommon.platform().craftingStationItem());
        CompoundTag tag = new CompoundTag();
        tag.putInt(TAG_TIER, targetTier);
        BlockEntity.addEntityType(tag, CraftingNetworkCommon.platform().craftingStationBlockEntityType());
    ...
    private ItemStack previousTierStack() {
        ItemStack stack = new ItemStack(CraftingNetworkCommon.platform().craftingStationItem());
        if (targetTier > 1) {
            CompoundTag tag = new CompoundTag();
    ...
    public static class Serializer implements RecipeSerializer<CraftingUpgradeRecipe> {
        public static final Serializer INSTANCE = new Serializer();

        private static final MapCodec<CraftingUpgradeRecipe> CODEC = RecordCodecBuilder.mapCodec(instance -> instance
                .group(Codec.INT.fieldOf("target_tier").forGetter(CraftingUpgradeRecipe::getTargetTier))
                .apply(instance, CraftingUpgradeRecipe::new));

        @Override
        public MapCodec<CraftingUpgradeRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, CraftingUpgradeRecipe> streamCodec() {
            return StreamCodec.composite(ByteBufCodecs.INT, CraftingUpgradeRecipe::getTargetTier,
                    CraftingUpgradeRecipe::new);
        }
    }
}