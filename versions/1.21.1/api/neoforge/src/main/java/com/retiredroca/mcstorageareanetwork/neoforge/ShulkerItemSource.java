...
public class ShulkerItemSource implements ItemSource {
    ...
    private static final class BoxRef {
        ...
        BoxRef(IItemHandler handler, BlockPos containerPos, int[] slots) {
        ...
        for (int dx = -chunkRadius; dx <= chunkRadius; dx++) {
            for (int dz = -chunkRadius; dz <= chunkRadius; dz++) {
                ...
                for (BlockEntity blockEntity : chunk.getBlockEntities().values()) {
                    if (!ContainerOwnership.canSee(level, ContainerOwnership.ownerOf(level, blockEntity.getBlockPos()), host)) {
                        continue;
                    }
                    for (net.minecraft.core.Direction side : net.minecraft.core.Direction.values()) {
                        IItemHandler handler = level.getCapability(
                                Capabilities.ItemHandler.BLOCK, blockEntity.getBlockPos(), side);
                        if (handler != null) {
                            scanHandler(boxRefs, handler, blockEntity.getBlockPos());
                            break;
                        }
                    }
                }
    ...
    private void scanHandler(List<BoxRef> boxRefs, IItemHandler handler, BlockPos containerPos) {
        for (int slot = 0; slot < handler.getSlots(); slot++) {
            ItemStack stack = handler.getStackInSlot(slot);
            if (isShulkerBox(stack)) {
                collectBoxes(boxRefs, handler, stack, new int[] { slot }, 1, containerPos);
            }
        }
    }
    ...
    private void collectBoxes(List<BoxRef> boxRefs, IItemHandler handler, ItemStack boxStack, int[] slots, int depth,
            BlockPos containerPos) {
        boxRefs.add(new BoxRef(handler, containerPos, slots));
        ...
        for (int s = 0; s < contents.size(); s++) {
            ...
            if (isShulkerBox(inner)) {
                ...
                next[slots.length] = s;
                collectBoxes(boxRefs, handler, inner, next, depth + 1, containerPos);
            }
        }
    }
    ...
    private ItemStack stackAt(BoxRef ref, int depth) {
        ItemStack stack = ref.handler.getStackInSlot(ref.slots[0]);
    ...
    private void writeBack(BoxRef ref, NonNullList<ItemStack> leafSlots) {
        ...
        setInSlot(ref.handler, ref.slots[0], node);
    }

    private static void setInSlot(IItemHandler handler, int slot, ItemStack stack) {
        ItemStack current = handler.getStackInSlot(slot);
        while (!current.isEmpty()) {
            current = handler.extractItem(slot, 64, false);
        }
        if (!stack.isEmpty()) {
